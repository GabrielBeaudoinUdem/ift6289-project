...
def main():
    parser = argparse.ArgumentParser("Qwen2.5 LoRA Training")
    parser.add_argument("--task", type=str, required=True, choices=["wiki", "fl"])
    args = parser.parse_args()

    MODEL_NAME = "Qwen/Qwen2.5-7B"
    OUTPUT_DIR = f"../models/qwen_lora_{args.task}"

    os.makedirs("../models", exist_ok=True)
    device = torch.device("mps") if torch.backends.mps.is_available() else (torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu"))
    print(f"Using device: {device} for Qwen training (Task: {args.task})")
