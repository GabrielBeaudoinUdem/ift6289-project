...
\maketitle
\begin{abstract}
Large language models exhibit strong performance accors many natural language
processing tasks, yet their understanding of fine-grained lexical relations
remains limited, unlike first tongue human speakers. Recent work has used the
Meaning-Text Theory (MTT) to evaluate models' lexical competence using
structured lexical functions, but the focus has been on evaluation rather than
model improvement. In this project, we propose a structured dataset of French
...
Modern natural language processing systems have achieved remarkable performance
...
appear to possess language understanding. This ability has been evaluated many
times, 
finding limits to their understanding of the language they use. Lexical
knowledge, the understanding of the words and of the relations between them, is
a field of application where LLMs tend to diverge from human performance.

The Meaning-Text theory (MTT)
\cite{melcukVersLinguistiqueSensTexte1997} decomposes language into
multiple 
components, from observable sound to abstract meaning: phonology, surface
...
other way corresponds to language generation. This theory allows for a
fine-grained description and analysis of multiple areas of linguistic
competency, and is meant to be universal to all languages.

As part of this framework, the relations between lexical units are expressed as
a mathematical concept, called lexical functions (LFs). For a given function
...
Structured lexical representations such as lexical functions might have the
...
This project aims to explain whether language models already encode lexical
...
functions as the primary input. We will then proceed to inject this knowledge into
pretrained language models. We will then evaluate the fine-tuned models on a
...
While general NLU benchmarks are usefull, they do not allow for a fine-grained
...
the authors found that tested language models generaly excel when needed to use
...
have few instances in fr-LN, we plan to set a minimum occurence threshold to
ensure consistensy.
...
"single lexical unit answer" condition. These metrics will require normalisation of the
